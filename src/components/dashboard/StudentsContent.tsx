    ...
    <div className="space-y-6">
      ...
      <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow p-6`}>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            <label className={`block text-sm font-medium mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Status Pilihan Jurusan</label>
            <select className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-green-500 focus:border-green-500 ${darkMode ? 'bg-gray-700 border-gray-600 text-gray-100' : 'bg-white border-gray-300 text-gray-900'}`}>
              <option value="" className={`${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>Semua</option>
              <option value="has_choice" className={`${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>Sudah Memilih</option>
              <option value="no_choice" className={`${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>Belum Memilih</option>
            </select>
          </div>
        </div>
      </div>
          ...
          <table className={`min-w-full divide-y ${darkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
            <thead className={darkMode ? 'bg-gray-700' : 'bg-gray-50'}>
              <tr>
                ...
                <th className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>Kontak</th>
                <th className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>Pilihan Jurusan</th>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        ...
                        <div className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>{student.nisn}</div>
                      </div>
                    </div>
                  </td>
                  <td className={`px-6 py-4 whitespace-nowrap text-sm ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>{student.class}</td>
                    ...
                    {student.has_choice ? (
                      <div>
                        ...
                        {student.chosen_major && (
                          <div className="text-xs text-gray-500 mt-1">{student.chosen_major.name}</div>
                        )}
                      </div>
                    ) : (
      ...
      <DeleteConfirmationModal
        ...
        itemName={selectedStudent ? `${selectedStudent.name} (${selectedStudent.nisn})` : undefined}
        darkMode={darkMode}
      />

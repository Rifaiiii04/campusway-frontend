...
export default function Sidebar({
  ...
  return (
    <div
      ...
      } shadow-xl transition-all duration-300 ease-in-out fixed left-0 top-0 h-full z-30 border-r`}
    >
      {/* Sidebar Header */}
      <div className="h-16 bg-gradient-to-r from-red-600 to-red-700 flex items-center px-3 shadow-lg relative">
        <button
          onClick={() => setSidebarOpen(!sidebarOpen)}
          className="text-white bg-red-700 hover:bg-red-800 p-2.5 rounded-lg transition-all duration-200 flex-shrink-0 hover:scale-110 active:scale-95 z-50 border-2 border-red-500 hover:border-red-400 shadow-lg hover:shadow-xl mr-3"
        ...
        >
          <h1 className="text-white text-lg font-bold tracking-wide whitespace-nowrap">
            ArahPotensi
          </h1>
        </div>
      </div>
        ...
        {menuItems.map((item) => (
          <button
            ...
            className={`w-full flex items-center px-3 py-3 text-left transition-all duration-300 rounded-lg mb-2 group relative ${
              activeMenu === item.id
                ? darkMode
                  ? "bg-gradient-to-r from-red-900 to-red-800 text-red-200 border-r-2 border-red-400 shadow-sm font-semibold"
                  : "bg-gradient-to-r from-red-50 to-red-100 text-red-700 border-r-2 border-red-600 shadow-sm font-semibold"
                : darkMode
          ...
          >
            <span className="text-xl mr-3 flex-shrink-0 transition-all duration-300">
              {item.icon}
            </span>
            <span
      ...
      {/* Sidebar Footer */}
      <div
        className={`absolute bottom-0 left-0 right-0 p-4 border-t shadow-inner ${
          darkMode ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-white"
        }`}
      >
        <div className="flex items-center">
          <div className="w-10 h-10 bg-gradient-to-br from-red-500 to-red-600 rounded-full flex items-center justify-center shadow-sm flex-shrink-0">
          ...
          {!sidebarOpen && (
            <div className="absolute left-full ml-2 px-2 py-1 bg-gray-900 text-white text-xs rounded opacity-0 hover:opacity-100 transition-opacity duration-200 whitespace-nowrap z-50">
              <p className="font-semibold">Guru ArahPotensi</p>
